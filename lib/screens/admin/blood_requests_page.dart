...
class _BloodRequestsPageState extends State<BloodRequestsPage> {
  ...
  Future<void> _sendNotification(String userId, String status, String userName) async {
      await _firestore.collection('notifications').add({
        'userId': userId,
        'title': 'Donation Offer Status',
        'body': 'Dear $userName, your generous offer to donate blood has been $status.',
        'timestamp': FieldValue.serverTimestamp(),
        'type': status, 
      });
  }

  // --- CRUD Operations (Approve/Reject Donors Offering) ---
  Future<void> _updateDonationOfferStatus(String docId, String newStatus, Map<String, dynamic> request) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Donation offer marked as $newStatus!"), 
  ...
  Widget _buildRequestCard(Map<String, dynamic> request, String docId) {
    ...
    final cardColor = isPending ? const Color(0xFFFFFBEB) : Colors.white; 
    final detailTextColor = isPending ? Colors.orange.shade900 : Colors.black87;
          ...
          children: [
            Expanded(
              child: ElevatedButton(
                onPressed: () => _approveOffer(docId, request), // Pass full request map
            ...
            Expanded(
              child: OutlinedButton(
                onPressed: () => _rejectOffer(docId, request), // Pass full request map
    ...
    return Card(
      ...
      shape: RoundedRectangleBorder(
        ...
        side: isPending ? const BorderSide(color: Colors.orange, width: 2) : BorderSide.none,
      ),
      elevation: 3,
      child: Padding(
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(request['name'] ?? 'N/A', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: detailTextColor)), 
                      Text(request['email'] ?? 'N/A', style: TextStyle(color: detailTextColor.withOpacity(0.7), fontSize: 14)),
                    ],
                  ),
                ),
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: isPending ? Colors.orange.shade100 : isApproved ? Colors.green.shade100 : Colors.red.shade100,
                    borderRadius: BorderRadius.circular(8),
                  ),
                  child: Text(status.toUpperCase(), style: TextStyle(color: isPending ? Colors.orange.shade800 : isApproved ? Colors.green.shade800 : Colors.red.shade800, fontWeight: FontWeight.bold, fontSize: 12)),
                )
              ],
            ),
            const SizedBox(height: 10),
            
            _buildDetailRow(Icons.phone, request['phone'] ?? 'N/A', detailTextColor),
            _buildDetailRow(Icons.calendar_today, "Donation Date: ${request['dateOfDonation'] ?? 'N/A'}", detailTextColor),
            _buildDetailRow(Icons.person, request['gender'] ?? 'N/A', detailTextColor),
            buttonRow,
          ],
        ),
      ),
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
            return const Center(child: Text('No new donation offers found.'));
          }
              ...
              SliverAppBar(
                ...
                title: Row(
                  children: [
                    ...
                    const Text("Donation Offers", style: TextStyle(color: Colors.black, fontSize: 20, fontWeight: FontWeight.bold)),
                  ],
                ),
              ),
                ...
                sliver: SliverList(
                  delegate: SliverChildListDelegate(
                    [
                      Row(
                        children: [
                          _buildStatCard("Total Offers", totalRequests.toString(), const Color(0xFFF94747), Icons.local_hospital),
                          const SizedBox(width: 12),
